...
// Edition VI · Champagne & Rose — service category tone map.
// The design system bans grayscale category colors and caps the palette
// at six tones (rose, foil, sage, honey, umber, slate). Each category
// picks from that palette; some reuse a tone where meaning aligns.
const catColors: Record<string, string> = {
  GOVERNMENT: "#A85A42",     // rose-deep — civic / formal
  UTILITY: "#E3B04B",        // honey — energy
  FINANCIAL: "#5EAD9A",      // sage — money / success
  HOUSING: "#B8936C",        // foil-c — home / warmth
  HEALTHCARE: "#D4846A",     // rose — care
  TRANSPORTATION: "#8AA9C0", // slate — movement / cool
  KIDS: "#EDB99D",           // rose-light — soft
  FITNESS: "#E5C9A8",        // foil — active / gold
  SHOPPING: "#8E6D4A",       // foil-ink — retail
  OTHER: "#D4B68F",          // umber — neutral
};
            ...
            <View style={{ marginBottom: 16, borderRadius: 16, borderWidth: 1, borderColor: "rgba(212, 132, 106,0.2)", backgroundColor: "rgba(212, 132, 106,0.04)", padding: 14 }}>
              ...
              {checklist.overdueItems.length > 0 && (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8, padding: 8, borderRadius: 8, backgroundColor: "rgba(239,68,68,0.08)" }}>
                  ...
                  <Text style={{ fontSize: 11, color: "#DC8470", flex: 1 }} numberOfLines={1}>
              ...
              {pending.map((item) => (
                <View key={item.id} style={{ flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 6 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {item.stateNote ? (
                      <Text style={{ fontSize: 10, color: "#EFC878" }} numberOfLines={2}>{item.stateNote}</Text>
                    ) : null}
                  </View>
                ...
                <View style={styles.serviceTop}>
                  ...
                  >
                    {service.monthlyCost > 0 ? (
                      ...
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: "rgba(227, 176, 75,0.1)", borderWidth: 1, borderColor: "rgba(227, 176, 75,0.25)" }}>
                        <DollarSign size={10} color="#EFC878" />
                        <Text style={{ fontSize: 10, fontWeight: "600", color: "#EFC878" }}>Add cost</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                </View>
