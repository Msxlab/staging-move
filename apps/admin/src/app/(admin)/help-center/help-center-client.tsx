  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        actions={
          <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"><Plus className="h-4 w-4" /> {tab === "articles" ? "New Article" : "New FAQ"}</button>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          ...
          { label: "FAQs", value: faqs.length, color: "text-foreground", bg: "bg-card" },
          { label: "Total Views", value: articles.reduce((s, a) => s + a.viewCount, 0), color: "text-tone-sky-fg", bg: "bg-tone-sky-bg" },
        ].map((s) => (
      ...
      <div className="flex gap-2 border-b border-border">
        {(["articles", "faqs"] as const).map(t => (<button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm font-semibold uppercase tracking-[0.18em] border-b-2 transition ${tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{t === "articles" ? "Articles" : "FAQs"}</button>))}
      </div>

      {showForm && tab === "articles" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          <h2 className="font-display text-lg font-bold text-foreground">{editing ? "Edit Article" : "New Article"}</h2>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Title</label><input value={articleForm.title} onChange={e => setArticleForm({ ...articleForm, title: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Slug</label><input value={articleForm.slug} onChange={e => setArticleForm({ ...articleForm, slug: e.target.value })} disabled={!!editing} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm font-mono text-foreground disabled:opacity-50" /></div>
            <div><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Category</label><input value={articleForm.category} onChange={e => setArticleForm({ ...articleForm, category: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Tags (comma-separated)</label><input value={articleForm.tags} onChange={e => setArticleForm({ ...articleForm, tags: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div className="col-span-2"><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Excerpt</label><input value={articleForm.excerpt} onChange={e => setArticleForm({ ...articleForm, excerpt: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div className="col-span-2"><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Content (Markdown)</label><textarea value={articleForm.content} onChange={e => setArticleForm({ ...articleForm, content: e.target.value })} rows={10} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground font-mono" /></div>
            <div className="flex items-center gap-4">
              ...
              <div><label className="text-sm text-muted-foreground mr-2">Order:</label><input type="number" value={articleForm.order} onChange={e => setArticleForm({ ...articleForm, order: parseInt(e.target.value) || 0 })} className="w-20 rounded-xl border border-input bg-background px-2 py-1 text-sm font-mono text-foreground" /></div>
            </div>
          </div>
          <div className="flex gap-2"><button onClick={saveArticle} className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90">{editing ? "Update" : "Create"}</button><button onClick={resetForm} className="rounded-xl border border-border px-4 py-2 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">Cancel</button></div>
        </div>
      )}

      {showForm && tab === "faqs" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          <h2 className="font-display text-lg font-bold text-foreground">{editing ? "Edit FAQ" : "New FAQ"}</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Question</label><input value={faqForm.question} onChange={e => setFaqForm({ ...faqForm, question: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div className="col-span-2"><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Answer</label><textarea value={faqForm.answer} onChange={e => setFaqForm({ ...faqForm, answer: e.target.value })} rows={4} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div><label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Category</label><input value={faqForm.category} onChange={e => setFaqForm({ ...faqForm, category: e.target.value })} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground" /></div>
            <div className="flex items-center gap-4">
              ...
              <div><label className="text-sm text-muted-foreground mr-2">Order:</label><input type="number" value={faqForm.order} onChange={e => setFaqForm({ ...faqForm, order: parseInt(e.target.value) || 0 })} className="w-20 rounded-xl border border-input bg-background px-2 py-1 text-sm font-mono text-foreground" /></div>
            </div>
          </div>
          <div className="flex gap-2"><button onClick={saveFaq} className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90">{editing ? "Update" : "Create"}</button><button onClick={resetForm} className="rounded-xl border border-border px-4 py-2 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">Cancel</button></div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {articles.length === 0 ? (<tr><td colSpan={6} className="px-4"><EmptyState icon={FileText} title="No articles yet" description="Create your first help article to get started." /></td></tr>) : articles.map(a => (
                <tr key={a.id} className="bg-card hover:bg-accent/30 transition-colors">
                  <td className="px-4 py-3"><p className="font-medium text-foreground">{a.title}</p><p className="text-xs font-mono text-muted-foreground">{a.slug}</p></td>
                  ...
                  <td className="px-4 py-3 font-mono text-muted-foreground">{a.viewCount}</td>
                  ...
                  <td className="px-4 py-3"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-semibold ${a.isPublished ? "bg-tone-sage-bg text-tone-sage-fg" : "bg-tone-honey-bg text-tone-honey-fg"}`}><span className={`h-1.5 w-1.5 rounded-full ${a.isPublished ? "bg-tone-sage-fg" : "bg-tone-honey-fg"}`} />{a.isPublished ? "Published" : "Draft"}</span></td>
      ...
      {tab === "faqs" && (
        <div className="space-y-3">
          {faqs.length === 0 ? (<div className="rounded-2xl border border-border bg-card"><EmptyState icon={MessageCircle} title="No FAQs yet" description="Add your first FAQ to help users find answers." /></div>) : faqs.map(f => (
            <div key={f.id} className="rounded-2xl border border-border bg-card p-4 transition-colors hover:bg-accent/30">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-semibold ${f.isPublished ? "bg-tone-sage-bg text-tone-sage-fg" : "bg-tone-honey-bg text-tone-honey-fg"}`}><span className={`h-1.5 w-1.5 rounded-full ${f.isPublished ? "bg-tone-sage-fg" : "bg-tone-honey-fg"}`} />{f.isPublished ? "Published" : "Draft"}</span>
                  </div>
