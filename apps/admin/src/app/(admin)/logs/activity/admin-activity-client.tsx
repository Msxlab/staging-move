...
export default function AdminActivityClient() {
  ...
  return (
    <div className="space-y-6">
      <Link href="/logs" className="inline-flex w-fit items-center gap-2 text-xs font-mono uppercase tracking-[0.18em] text-muted-foreground transition-colors hover:text-foreground">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Total (All Time)</p>
          <p className="mt-1.5 font-display text-3xl font-extrabold leading-none text-foreground">{stats?.totalAll?.toLocaleString() || 0}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Last 30 Days</p>
          <p className="mt-1.5 font-display text-3xl font-extrabold leading-none text-foreground">{stats?.total30d?.toLocaleString() || 0}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Last 7 Days</p>
          <p className="mt-1.5 font-display text-3xl font-extrabold leading-none text-foreground">{stats?.total7d?.toLocaleString() || 0}</p>
        </div>
      </div>
                  ...
                  <div key={item.admin.id}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="w-5 font-mono text-xs font-bold text-muted-foreground">#{i + 1}</span>
                        <div>
                      ...
                      <span className="font-mono text-sm font-bold text-foreground">{item.actions}</span>
                    </div>
                  ...
                  <div key={a.action} className="flex items-center gap-3">
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className={`font-mono text-sm ${isDangerous ? "font-medium text-destructive" : "text-foreground"}`}>{a.action}</span>
            ...
            <div className="flex items-end gap-[2px] h-32">
              {dailyActivity.map((d: any) => (
                <div key={d.date} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                  ...
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block rounded-lg border border-border bg-card px-2 py-1 font-mono text-[10px] text-foreground whitespace-nowrap shadow-lg z-10">
        ...
        <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6">
          ...
          <div className="space-y-2">
            {criticalActions.map((c: any) => (
              <div key={c.id} className="flex items-center justify-between rounded-xl border border-border bg-card/80 p-3">
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-destructive/10 px-2.5 py-0.5 font-mono text-xs font-medium text-destructive">
                    <span className="h-1.5 w-1.5 rounded-full bg-destructive" />
                    {c.action}
                  </span>
                  <div>
                    <p className="text-sm text-foreground">{c.entityType} · <span className="font-mono text-xs">{c.entityId}</span></p>
                    {c.admin && <p className="text-[10px] text-muted-foreground">{c.admin.firstName} {c.admin.lastName}</p>}
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-mono text-xs text-muted-foreground">{new Date(c.createdAt).toLocaleString()}</p>
                  {c.ipAddress && <p className="font-mono text-[10px] text-muted-foreground">{c.ipAddress}</p>}
                </div>
              </div>
            ))}
          </div>
