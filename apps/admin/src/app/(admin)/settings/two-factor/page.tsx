...
const inputCls = "w-full rounded-xl border border-input bg-background px-4 py-2.5 text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20";
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {showEnrollmentBanner && (
        <div
          role="alert"
          className="rounded-2xl border border-tone-honey-br bg-tone-honey-bg p-5 flex items-start gap-3"
        >
          <div className="rounded-xl bg-background p-2 mt-0.5">
      ...
      {/* Current Status */}
      <div className={`rounded-2xl border p-6 ${status === "enabled" ? "border-tone-sage-br bg-tone-sage-bg" : "border-border bg-card"}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {status === "enabled" ? (
              ...
              <div className="rounded-xl bg-tone-honey-bg p-2.5">
          ...
          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${status === "enabled" ? "bg-tone-sage-bg text-tone-sage-fg" : "bg-tone-honey-bg text-tone-honey-fg"}`}>
            <span className={`h-1.5 w-1.5 rounded-full ${status === "enabled" ? "bg-tone-sage-fg" : "bg-tone-honey-fg"}`} />
      ...
      {status === "disabled" && setupStep === "idle" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          <h2 className="font-display text-lg font-bold text-foreground flex items-center gap-2">
            <KeyRound className="h-5 w-5 text-primary" /> Enable Two-Factor Authentication
          </h2>
          <p className="text-sm text-muted-foreground">
      ...
      {setupStep === "confirming" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          <h2 className="font-display text-lg font-bold text-foreground">Confirm Your Password</h2>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setSetupStep("idle"); setConfirmPassword(""); }}
              className="rounded-xl border border-border px-5 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">
              Cancel
            </button>
          </div>
        </div>
      ...
      {setupStep === "scanning" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
          <h2 className="font-display text-lg font-bold text-foreground">Scan QR Code</h2>
          <p className="text-sm text-muted-foreground">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="w-full rounded-xl bg-muted/30 p-4 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-mono font-semibold uppercase tracking-[0.18em] text-muted-foreground">Manual Entry Key</p>
          ...
          <div className="rounded-xl border border-tone-honey-br bg-tone-honey-bg p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={downloadBackupCodes}
                className="flex items-center gap-1 rounded-xl border border-border bg-background px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">
            ...
            <div className="grid grid-cols-2 gap-2">
              {backupCodes.map((code, i) => (
                <div key={i} className="flex items-center justify-between rounded-xl bg-background px-3 py-2">
                  <span className="font-mono text-sm text-foreground">{code}</span>
                  <button onClick={() => copyToClipboard(code, "Code")} aria-label="Copy backup code" className="text-muted-foreground transition-colors hover:text-foreground">
      ...
      {setupStep === "verifying" && (
        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          <h2 className="font-display text-lg font-bold text-foreground">Verify Setup</h2>
          <p className="text-sm text-muted-foreground">
      ...
      {/* Step 4: Done */}
      {setupStep === "done" && (
        <div className="rounded-2xl border border-tone-sage-br bg-tone-sage-bg p-6 text-center space-y-3">
          <CheckCircle2 className="h-12 w-12 text-tone-sage-fg mx-auto" />
          <h2 className="font-display text-lg font-bold text-foreground">Two-Factor Authentication Enabled</h2>
          <p className="text-sm text-muted-foreground">
            Your account is now secured with an authenticator app. You&apos;ll need to enter a code each time you sign in.
          </p>
          <Link href="/settings" className="inline-block rounded-xl border border-border bg-background px-5 py-2.5 text-sm font-semibold text-foreground transition-colors hover:bg-accent">
            Back to Settings
          </Link>
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            ...
            {!showDisable && (
              <button onClick={() => setShowDisable(true)}
                className="rounded-xl border border-destructive/30 px-4 py-2 text-sm font-semibold text-destructive transition-colors hover:bg-destructive/10">
                Disable 2FA
              </button>
            )}
          </div>
            ...
            <div className="space-y-3 pt-2 border-t border-border">
              ...
              <div className="flex gap-2">
                <button onClick={handleDisable} disabled={disabling || !disablePassword}
                  className="flex items-center gap-2 rounded-xl bg-destructive px-5 py-2.5 text-sm font-semibold text-destructive-foreground transition-colors hover:bg-destructive/90 disabled:opacity-50">
