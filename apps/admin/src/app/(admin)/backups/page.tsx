...
import {
  Database, Download, Upload, Loader2, CheckCircle2, XCircle, Clock,
  HardDrive, FileJson, Table2, RefreshCw, AlertTriangle, FileUp,
} from "lucide-react";
...
const BACKUP_TYPES = [
  ...
  { value: "USERS", label: "Users Only", desc: "Users, profiles, subscriptions" },
...
function extractImportPreview(parsed: any) {
  ...
  if (archive) {
    ...
    const preview = Object.fromEntries(tables.map((table: string) => [table, typeof metadataCounts[table] === "number" ? metadataCounts[table] : 0]));
...
function LegacyBackupsPage() {
  ...
  async function submitWithPassword(url: string, payload: Record<string, unknown>, fallbackError: string) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (res.status === 403 && data?.requiresPassword && !requestBody.confirmPassword) {
        const confirmPassword = window.prompt("Confirm your admin password to continue this backup operation.");
        if (!confirmPassword) {
  ...
  function downloadBackup(downloadData: string, fileName: string) {
  ...
  async function createBackup() {
    ...
    try {
      ...
      if (data.downloadData) {
        downloadBackup(data.downloadData, data.backup?.fileName || "backup.json");
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Export and manage system data backups</p>
        </div>
        <button
          onClick={loadBackups}
      ...
      {storage && (
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-foreground">Offsite Backup Storage</p>
              ...
              {storage.endpoint && <p className="mt-1 text-[11px] text-muted-foreground">Endpoint: {storage.endpoint}</p>}
            </div>
            <span className={`rounded-full px-3 py-1 text-xs font-medium ${storage.ready ? "bg-green-500/10 text-green-500" : storage.unsupportedProvider ? "bg-red-500/10 text-red-500" : storage.configured ? "bg-amber-500/10 text-amber-500" : "bg-slate-500/10 text-slate-500"}`}>
              {storage.ready ? "Ready" : storage.unsupportedProvider ? "Unsupported" : storage.configured ? "Needs credentials" : "Not configured"}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          {Object.entries(stats).map(([table, count]) => (
            <div key={table} className="rounded-lg bg-muted/30 p-3 text-center">
              ...
              <p className="text-[10px] text-muted-foreground capitalize">{table.replace(/([A-Z])/g, " $1").trim()}</p>
            </div>
          ))}
        </div>
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            Total: {totalRecords.toLocaleString()} records across {Object.keys(stats).length} tables
          </div>
          {backups.length > 0 && (() => {
            const lastBackup = backups[0];
            const daysSince = Math.floor((Date.now() - new Date(lastBackup.createdAt).getTime()) / (1000 * 60 * 60 * 24));
              ...
              <div className="flex items-center gap-3 text-xs">
                {diff !== 0 && (
                  <span className={diff > 0 ? "text-emerald-400" : "text-red-400"}>
                    {diff > 0 ? "+" : ""}{diff} records since last backup
                  </span>
                )}
                <span className={`flex items-center gap-1 ${daysSince > 7 ? "text-amber-400" : "text-muted-foreground"}`}>
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-2">Backup Type</label>
            <div className="flex flex-wrap gap-2">
              {BACKUP_TYPES.map((bt) => (
                ...
                >
                  {bt.label}
                  <span className="block text-[10px] opacity-70">{bt.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {selectedType !== "FULL" && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-2">Select Tables</label>
              <div className="flex flex-wrap gap-2">
                {allTables.map((t) => (
                  <button
                    key={t}
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-4">
          {/* File Upload */}
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-2">Upload Backup File (JSON)</label>
            <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-border rounded-xl cursor-pointer hover:bg-muted/20 transition">
              ...
              <span className="text-sm text-muted-foreground">
                {importFile ? importFile.name : "Click to select backup JSON file"}
              </span>
              <input
                type="file"
                accept=".json"
            ...
            <>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-2">
                  File Contents ({Object.keys(importPreview).length} tables, {Object.values(importPreview).reduce((a, b) => a + b, 0).toLocaleString()} records)
                </label>
                {importIsEncrypted && (
                  <p className="mb-2 text-[11px] text-muted-foreground">Encrypted archive preview uses embedded metadata counts until the file is decrypted on the server.</p>
                )}
                <div className="flex flex-wrap gap-2">
                  {Object.entries(importPreview).map(([table, count]) => (
                    <button
                      key={table}
                      onClick={() => setImportTables((prev) =>
              ...
              {/* Mode */}
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-2">Import Mode</label>
                <div className="flex gap-2">
                  <button
                    onClick={() => setImportMode("MERGE")}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                      importMode === "MERGE" ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground hover:bg-muted"
                    }`}
                  >
                    Merge
                    <span className="block text-[10px] opacity-70">Skip existing records</span>
                  </button>
                  <button
                    onClick={() => setImportMode("REPLACE")}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                      importMode === "REPLACE" ? "bg-destructive text-destructive-foreground" : "bg-muted/50 text-muted-foreground hover:bg-muted"
                    }`}
                  >
                    Replace
                    <span className="block text-[10px] opacity-70">Delete & re-insert</span>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <button
                  onClick={() => { setImportFile(null); setImportPayload(null); setImportPreview(null); setImportResult(null); setImportTables([]); setImportIsEncrypted(false); }}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      toast.success(`Imported ${result.summary?.totalImported || 0} records`);
                      await loadBackups();
                    } catch (error: any) {
                ...
                >
                  {importing ? (
                    ...
                    <><Upload className="h-4 w-4" /> Import {importTables.length} Tables</>
                  )}
                </button>
              </div>
                ...
                <div className="rounded-lg bg-muted/30 p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="rounded-lg bg-emerald-500/10 p-2">
                      <p className="text-lg font-bold text-emerald-400">{importResult.summary?.totalImported || 0}</p>
                      <p className="text-[10px] text-muted-foreground">Imported</p>
                    </div>
                    <div className="rounded-lg bg-yellow-500/10 p-2">
                      <p className="text-lg font-bold text-yellow-400">{importResult.summary?.totalSkipped || 0}</p>
                      <p className="text-[10px] text-muted-foreground">Skipped</p>
                    </div>
                    <div className="rounded-lg bg-red-500/10 p-2">
                      <p className="text-lg font-bold text-red-400">{importResult.summary?.totalErrors || 0}</p>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1 mt-2">
                      {Object.entries(importResult.results).map(([table, r]: [string, any]) => (
                        <div key={table} className="flex justify-between">
                          <span>{table}</span>
                          <span>{r.imported} imported, {r.skipped} skipped{r.errors > 0 ? `, ${r.errors} errors` : ""}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {backups.length === 0 ? (
          ...
          <div className="space-y-2">
            {backups.map((backup) => {
              ...
              const statusColor = STATUS_COLORS[backup.status] || "text-gray-500 bg-gray-500/10";
              const tables = backup.tables ? JSON.parse(backup.tables) : [];
              const offsiteColor = backup.offsite?.status === "stored"
                ? "text-green-500 bg-green-500/10"
                : backup.offsite?.status === "failed"
                  ? "text-red-500 bg-red-500/10"
                  : "text-slate-500 bg-slate-500/10";
              const offsiteLabel = backup.offsite?.status === "stored"
                ? "Offsite stored"
                : backup.offsite?.status === "failed"
                  ? "Offsite failed"
                  : "Offsite disabled";
              return (
                <div key={backup.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 hover:bg-muted/50 transition">
                  <div className="flex items-center gap-4">
                    <div className={`p-2 rounded-lg ${statusColor}`}>
                      <StatusIcon className={`h-4 w-4 ${backup.status === "IN_PROGRESS" ? "animate-spin" : ""}`} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-foreground">{backup.fileName || `backup-${backup.id}`}</p>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        ...
                        {backup.recordCount !== undefined && <span>{backup.recordCount.toLocaleString()} records</span>}
                        {backup.fileSize && <span>{formatBytes(backup.fileSize)}</span>}
                        {tables.length > 0 && <span>{tables.length} tables</span>}
                      </div>
                      {(backup.offsite?.location || backup.offsite?.reason || backup.errorMessage) && (
                        <p className={`mt-1 text-[11px] ${backup.offsite?.status === "failed" || backup.status === "FAILED" ? "text-red-500" : "text-muted-foreground"}`}>
                          {backup.offsite?.reason || backup.errorMessage || backup.offsite?.location}
                        </p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {backup.offsite && (
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${offsiteColor}`}>
                        {offsiteLabel}
                      </span>
                    )}
                  </div>
