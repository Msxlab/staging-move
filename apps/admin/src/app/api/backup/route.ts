...
import { getBackupStorageSummary, parseBackupRecordMetadata, serializeBackupRecordMetadata, uploadBackupArchive } from "@/lib/backup-storage";
import { encryptBackup, signBackup } from "@/lib/shared-encryption";

const BACKUP_TABLES: Record<string, { model: string; label: string }> = {
  ...
  documents: { model: "document", label: "Documents" },
  badges: { model: "badge", label: "Badges" },
...
const BACKUP_TABLE_OPS = {
  users: { count: () => prisma.user.count(), findRecords: () => prisma.user.findMany({ take: 50000 }) },
  profiles: { count: () => prisma.profile.count(), findRecords: () => prisma.profile.findMany({ take: 50000 }) },
  addresses: { count: () => prisma.address.count(), findRecords: () => prisma.address.findMany({ take: 50000 }) },
  services: { count: () => prisma.service.count(), findRecords: () => prisma.service.findMany({ take: 50000 }) },
  providers: { count: () => prisma.serviceProvider.count(), findRecords: () => prisma.serviceProvider.findMany({ take: 50000 }) },
  movingPlans: { count: () => prisma.movingPlan.count(), findRecords: () => prisma.movingPlan.findMany({ take: 50000 }) },
  tasks: { count: () => prisma.task.count(), findRecords: () => prisma.task.findMany({ take: 50000 }) },
  budgets: { count: () => prisma.budget.count(), findRecords: () => prisma.budget.findMany({ take: 50000 }) },
  subscriptions: { count: () => prisma.subscription.count(), findRecords: () => prisma.subscription.findMany({ take: 50000 }) },
  auditLogs: { count: () => prisma.auditLog.count(), findRecords: () => prisma.auditLog.findMany({ take: 50000 }) },
  notifications: { count: () => prisma.notification.count(), findRecords: () => prisma.notification.findMany({ take: 50000 }) },
} as const;
...
export async function GET() {
  try {
    ...
    for (const backup of backups as Array<{ createdBy?: unknown }>) {
      if (typeof backup.createdBy === "string" && backup.createdBy !== "CRON" && !creatorIds.includes(backup.createdBy)) {
        creatorIds.push(backup.createdBy);
      }
    }
    const adminUsers = creatorIds.length > 0
    ...
    for (const key of Object.keys(BACKUP_TABLES)) {
      try {
    ...
    return NextResponse.json({
      backups: backups.map((backup: any) => normalizeBackupRecord(
        backup,
        backup.createdBy === "CRON" ? "CRON" : creatorLabels.get(backup.createdBy) || backup.createdBy
      )),
      stats,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { type = "FULL", tables = [], format = "JSON", confirmPassword } = body;
    ...
    const backup = await prisma.backupRecord.create({
      data: {
        ...
        status: "IN_PROGRESS",
        format,
        tables: JSON.stringify(tables.length > 0 ? tables : Object.keys(BACKUP_TABLES)),
    ...
    // Collect data
    const selectedTables = tables.length > 0 ? tables : Object.keys(BACKUP_TABLES);
    const backupData: Record<string, any[]> = {};
    const tableCounts: Record<string, number> = {};
    let totalRecords = 0;

    for (const tableName of selectedTables) {
      const tableOps = BACKUP_TABLE_OPS[tableName as keyof typeof BACKUP_TABLE_OPS];
    ...
    await prisma.adminAuditLog.create({
      data: {
        ...
        entityId: backup.id,
        changes: JSON.stringify({ type, tables: selectedTables, recordCount: totalRecords, fileSize, isEncrypted: !!encrypted, signature: !!signature, offsiteStatus: offsite.status, offsiteLocation: offsite.location }),
    ...
    return NextResponse.json({
      ...
      offsite,
      ...(encrypted ? { encryptedData: encrypted.encryptedData, iv: encrypted.iv, authTag: encrypted.authTag } : {}),
    }, { status: 201 });
  } catch (error: any) {
    if (backupId) {
      await prisma.backupRecord.update({
        ...
        data: {
          status: "FAILED",
          errorMessage: serializeBackupRecordMetadata({ error: error?.message || "Failed to create backup." }),
        },
      }).catch(() => null);
    }
    if (error?.message === "UNAUTHORIZED") {
