...
describe("monthlyEquivalentUsd", () => {
  it("prices a monthly paid plan at its monthly price", () => {
    expect(monthlyEquivalentUsd(sub({ plan: "FAMILY" }))).toBeCloseTo(9.99);
  });

  it("amortizes an annual plan to monthly", () => {
    expect(monthlyEquivalentUsd(sub({ plan: "INDIVIDUAL", billingInterval: "YEAR" }))).toBeCloseTo(39.99 / 12);
  });
...
describe("computeMrr", () => {
  it("sums monthly equivalents over active, paying subs only", () => {
    const subs = [
      sub({ plan: "INDIVIDUAL" }), // 3.99
      sub({ plan: "FAMILY" }), // 9.99
      sub({ status: "TRIALING" }), // 0
      sub({ status: "CANCELED" }), // not active → 0
    ];
    expect(computeMrr(subs)).toBeCloseTo(13.98);
  });
});
...
describe("computeArpuByPlan", () => {
  it("breaks ARPU down per plan, excluding trials from the paying denominator", () => {
    ...
    expect(individual.mrr).toBeCloseTo(7.98);
    expect(individual.arpu).toBeCloseTo(3.99);
    // Sorted by MRR desc: FAMILY (9.99) before INDIVIDUAL (7.98).
...
describe("computeMrrMovement", () => {
  ...
  it("counts new MRR from subs created in the window and churned MRR from those canceled in it", () => {
    const subs = [
      sub({ plan: "INDIVIDUAL", createdAt: new Date("2026-05-15T00:00:00.000Z") }), // +3.99 new
    ...
    expect(m.newMrr).toBeCloseTo(3.99);
    expect(m.churnedMrr).toBeCloseTo(19.99);
    expect(m.netMrr).toBeCloseTo(3.99 - 19.99);
  });
});
...
describe("computeMrrTrend", () => {
  it("returns one estimated point per trailing month, oldest first", () => {
    ...
    expect(trend[2].mrr).toBeCloseTo(3.99);
  });
