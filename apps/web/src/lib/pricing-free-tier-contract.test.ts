...
describe("consumer-free pricing + affiliate contract", () => {
  ...
  it("keeps the source paper app canvas with white web surfaces", () => {
    ...
    expect(globals).toMatch(/\.lf-app-shell\[data-lf-theme="light"\]\s*\{[\s\S]*background:\s*var\(--lf-app-bg,\s*#EFEADF\) !important;/);
    ...
    expect(aurora).toMatch(/\.light \.lf-aurora\s*\{[\s\S]*--au-base:\s*#EFEADF;/);
    ...
    expect(globals).toContain("--lf-app-bg: #EFEADF");
