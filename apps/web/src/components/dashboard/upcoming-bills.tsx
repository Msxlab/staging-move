...
function getBillStatus(
  ...
  if (daysUntil === 0) return { label: td("bills_dueToday"), color: "text-destructive", icon: AlertTriangle };
  if (daysUntil <= 3) return { label: td("bills_dueIn", { days: daysUntil }), color: "text-tone-honey-fg", icon: Clock };
  if (daysUntil <= 7) return { label: td("bills_dueIn", { days: daysUntil }), color: "text-tone-cyan-fg", icon: Calendar };
    ...
    <div className="rounded-2xl border border-border bg-foreground/5 backdrop-blur-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-5 pb-3">
        <div className="flex items-center gap-2">
          <Receipt className="h-4 w-4 text-tone-honey-fg" />
          <h3 className="text-sm font-semibold text-foreground">{td("widget_bills")}</h3>
        </div>
        {upcomingThisWeek.length > 0 && (
          <span className="text-xs font-semibold text-tone-honey-fg">
            ...
            return (
              <Link key={bill.id} href={`/services/${bill.id}`}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-border bg-foreground/[0.02] hover:bg-foreground/[0.05] transition group cursor-pointer">
                  <div className={`p-1.5 rounded-lg ${daysUntil <= 3 ? "bg-tone-honey-bg border border-tone-honey-br" : "bg-foreground/5"}`}>
