          ...
          bills.map((bill) => {
            ...
            return (
              <Link key={bill.id} href={`/services/${bill.id}`}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition group cursor-pointer">
                  <div className={`p-1.5 rounded-lg ${daysUntil <= 3 ? "bg-amber-500/10 border border-amber-500/20" : "bg-white/5"}`}>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs font-semibold text-white/60">{formatCurrency(bill.monthlyCost)}</p>
