    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="h-32 rounded-2xl border border-white/5 bg-white/[0.02] animate-pulse" />
        </div>
        <div className="space-y-5">
          <div className="h-40 rounded-2xl border border-white/5 bg-white/[0.02] animate-pulse" />
          <div className="h-36 rounded-2xl border border-white/5 bg-white/[0.02] animate-pulse" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="rounded-xl border border-white/5 bg-white/[0.02] overflow-hidden">
      <div className="grid gap-0" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
        {Array.from({ length: cols }).map((_, i) => (
          <div key={`h-${i}`} className="h-10 bg-white/[0.03] border-b border-white/5 animate-pulse" />
        ))}
        {Array.from({ length: rows * cols }).map((_, i) => (
          <div key={i} className="h-12 border-b border-white/[0.02] animate-pulse" />
        ))}
      </div>
    </div>
  );
