...
export function DeleteAccountDialog({
  ...
  const upperTyped = trimmedTyped.toUpperCase();
  const typedOk =
    upperTyped === "DELETE" ||
    upperTyped === "ELIMINAR" ||
    (normalizedEmail.length > 0 && trimmedTyped.toLowerCase() === normalizedEmail);
  const passwordRequired = hasPasswordLogin;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/account/delete", {
        ...
        body: JSON.stringify({
          confirmText: trimmedTyped,
          ...(passwordRequired ? { confirmPassword: password } : {}),
          ...(!passwordRequired && !showMfa ? { confirmAccountDeletion: true } : {}),
        ...
        {step === "warning" ? (
          <>
            ...
            <ul className="space-y-1.5 rounded-xl border border-destructive bg-destructive/5 p-4 text-xs text-muted-foreground">
              ...
              {!passwordRequired && <li>Google or Apple sign-in accounts do not need a password setup step</li>}
          ...
          <>
            ...
            {!passwordRequired && !showMfa && (
              <div className="rounded-xl border border-tone-honey-br bg-tone-honey-bg p-3 text-xs leading-5 text-tone-honey-fg/80">
