...
const SETTINGS_COPY = {
  en: {
    ...
    cards: {
      ...
      // Free-pivot copy: shown when CONSUMER_FREE is on. Reversible — the flag
      // OFF path renders the original "Subscription ready" string above.
      planValueFree: "Free · everything included",
    ...
    // Free-pivot override for the Subscription quick-link. Same href to the
    // already-free-aware /settings/subscription page; only label + description change.
    subscriptionFree: { title: "Your plan", description: "Free — everything included" },
    danger: {
      title: "Danger Zone",
  ...
  es: {
    ...
    cards: {
      ...
      planValueFree: "Gratis · todo incluido",
    ...
    subscriptionFree: { title: "Tu plan", description: "Gratis — todo incluido" },
    danger: {
...
export default function SettingsClient({ consumerFree = false }: { consumerFree?: boolean }) {
  ...
  // Free-pivot copy is byte-identical to today when CONSUMER_FREE is OFF — the
  // ternaries below fall through to the original paid-model strings.
  const planValue = consumerFree ? copy.cards.planValueFree : copy.cards.planValue;
  const sections: readonly SettingsSection[] = consumerFree
    ? copy.sections.map((section) =>
        section.href === "/settings/subscription"
          ? { ...section, title: copy.subscriptionFree.title, description: copy.subscriptionFree.description }
          : section,
      )
    : copy.sections;
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-card/80 shadow-sm">
        ...
        <div className="grid gap-3 p-4 sm:grid-cols-3">
          <div className="rounded-xl border border-border bg-background/55 p-3">
            ...
            <p className="mt-1 font-display text-sm font-semibold text-foreground">{planValue}</p>
          </div>
          <div className="rounded-xl border border-border bg-background/55 p-3">
