...
// Under CONSUMER_FREE every consumer resolves to PRO (seat limit 10), so a newly
// created workspace can invite up to (limit - 1) others. Sourced from the shared
// seat helper so it tracks the PRO ceiling rather than a hardcoded copy number.
const SHARED_WORKSPACE_INVITE_COUNT = seatLimitForPlan("PRO") - 1;
...
export default function WorkspaceSettingsClient({ consumerFree = false }: { consumerFree?: boolean }) {
  ...
  // create affordance in the empty state. When the flag is OFF these stay unused
  // and the paid copy / no-create behavior is byte-identical to before.
  ...
  // Create a shared workspace (consumer-free only). POST /api/workspaces is
  // already consumer-free-aware (resolves PRO under CONSUMER_FREE), so this
  // succeeds for a free user. On success, load the new workspace into page
  // state and select it — mirroring the existing fetch/error patterns.
  const createWorkspace = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || "Couldn't create the workspace.");
        return;
      }
      toast.success("Workspace created.");
      setCreateName("");
      // Re-fetch the full list so the new workspace arrives with the same
      // shape (planLabel, seatLimit, memberCount, isPersonalSolo) the page
      // relies on, then select it.
      const newId: string | null = typeof data?.id === "string" ? data.id : null;
      try {
        const wsRes = await fetch("/api/workspaces", { cache: "no-store" });
        const wsData = await wsRes.json().catch(() => ({}));
        const list: Workspace[] = Array.isArray(wsData.workspaces) ? wsData.workspaces : [];
        setWorkspaces(list);
        const nextId = list.find((w) => w.id === newId)?.id ?? newId ?? list[0]?.id ?? null;
        if (nextId) {
          setSelectedId(nextId);
          persistWorkspaceSelection(nextId);
        }
      } catch {
        // The workspace was created server-side; a reload will pick it up.
        if (newId) {
          setSelectedId(newId);
          persistWorkspaceSelection(newId);
        }
      }
    } catch {
        ...
        {consumerFree && workspaceModelEnabled ? (
          <div className="rounded-2xl border border-border bg-foreground/5 p-6">
            <p className="text-sm text-muted-foreground">
              ...
              {SHARED_WORKSPACE_INVITE_COUNT} people and share your move — included free.
            </p>
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-8">
      ...
      {selected && (
        <>
          ...
          {/* Consumer-free: a create-workspace affordance for a user who only has
          ...
          {consumerFree && workspaceModelEnabled && iAmManager && selected.isPersonalSolo && (
            <div className="rounded-2xl border border-border bg-foreground/5 p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Invite up to {SHARED_WORKSPACE_INVITE_COUNT} people and share your move — included free.
                Everyone keeps their own login and private data.
              </p>
