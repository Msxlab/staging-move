    ...
    const [services, total] = await Promise.all([
      prisma.service.findMany({
        ...
        include: {
          ...
          provider: { select: { id: true, name: true, slug: true, website: true, phone: true, scope: true, avgRating: true, reviewCount: true } },
        },
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Auto-complete matching checklist tasks (transactional) ──
    try {
      await prisma.$transaction(async (tx: any) => {
        ...
        if (matchingTasks.length > 0) {
          await tx.task.updateMany({
            where: {
              id: { in: matchingTasks.map((t: any) => t.id) },
            },
            data: {
              completed: true,
          ...
          const planIds = [...new Set(matchingTasks.map((t: any) => t.movingPlanId).filter(Boolean))];
          for (const planId of planIds) {
            const completedCount = await tx.task.count({
              where: { movingPlanId: planId!, completed: true },
            });
      ...
      console.error("Auto-complete task failed (non-blocking):", autoErr);
    }
