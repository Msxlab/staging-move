...
Reason: local component-level visual QA passed, but full authenticated staging/browser verification could not be completed from this environment. Chrome CDP was not available earlier, and staging must rebuild/deploy the latest PR commit before the user-facing page can be judged.
...
- Light canvas is now warm paper rather than the darker greige wash reported from staging.
