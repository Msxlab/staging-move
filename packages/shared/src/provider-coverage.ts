// Pure utilities for ServiceProvider coverage — ZIP ↔ state lookup,
// JSON-string parsing, and expansion of (scope, states, zipCodes) into
// normalized ServiceProviderCoverage rows. Used by both the web backend
// (to seed the coverage table) and any client that needs ZIP → state.
//
...
export function normalizeZipRule(value: string): string {
...
export function zipToState(zip: string): string | undefined {
...
// - STATE with no zipCodes → one whole-state row per listed state.
// - STATE with zipCodes → per-zip rows; unresolvable zips are skipped.
export function expandCoverageRows(input: {
  scope: string;
  states?: string[] | string | null;
  zipCodes?: string[] | string | null;
}): CoverageRow[] {
  const scope = (input.scope || "FEDERAL").toUpperCase();
  const states = (Array.isArray(input.states) ? input.states : safeJsonArray(input.states))
    .map((s) => s.trim().toUpperCase())
    .filter((s) => s.length === 2);
  const rawZips = Array.isArray(input.zipCodes) ? input.zipCodes : safeJsonArray(input.zipCodes);
  const normalizedZips = rawZips.map(normalizeZipRule).filter((z) => z.length >= 3 && z.length <= 5);

  if (scope === "FEDERAL") {
    if (normalizedZips.length === 0) return [];
    const rows: CoverageRow[] = [];
    for (const zip of normalizedZips) {
      const state = zipToState(zip);
      if (!state) continue;
      rows.push(
        zip.length === 5
          ? { state, zipPrefix: null, zipExact: zip }
          : { state, zipPrefix: zip, zipExact: null }
      );
    }
    return rows;
  }
  ...
  if (normalizedZips.length === 0) {
    return states.map((state) => ({ state, zipPrefix: null, zipExact: null }));
  }
  ...
  for (const zip of normalizedZips) {
    const state = zipToState(zip);
    if (!state) continue;
    if (statesSet.size > 0 && !statesSet.has(state)) continue;
    rows.push(
      zip.length === 5
        ? { state, zipPrefix: null, zipExact: zip }
        : { state, zipPrefix: zip, zipExact: null }
