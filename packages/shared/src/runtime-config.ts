...
export const RUNTIME_CONFIG_DEFINITIONS: readonly RuntimeConfigDefinition[] = [
  ...
  {
    key: "CPL_CENTS_CLEANING",
    label: "Cleaning CPL (cents)",
    description: "Cost-per-lead charged to a cleaning partner for each delivered lead, in US cents (e.g. 1500 = $15.00). Unset or ≤0 means leads are delivered free (no charge accrues). Changing it never retro-bills already-accrued leads. (R5)",
    scope: "WEB",
    category: "BILLING",
    isSecret: false,
    requiredInProduction: false,
    maskStrategy: "plain",
    runtimeEditable: true,
    usedBy: ["web lead-gen billing accrual"],
    note: "Partner-billing only; consumers are never charged. Live invoicing (R5c) also requires legal/tax sign-off.",
  },
  {
    key: "CPL_CENTS_JUNK",
    label: "Junk-removal CPL (cents)",
    description: "Cost-per-lead charged to a junk-removal partner for each delivered lead, in US cents. Unset or ≤0 means free (no charge accrues). (R5)",
    scope: "WEB",
    category: "BILLING",
    isSecret: false,
    requiredInProduction: false,
