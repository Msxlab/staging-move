...
- Auth fetch loop: auth pages and global session tracking could both check `/api/auth/me`; prior hotfix introduced one-shot auth checks on sign-in/sign-up and disabled `SessionTracker` current-user polling on auth pages.
...
- Added SUPER_ADMIN-only restore action with password confirmation.
...
- Auth page regression:
  ...
  - sign-in/sign-up auth checks are guarded from retry loops
...
8. Soft-deleted user: password and OAuth login remain blocked until admin restore.
...
- Restore a soft-deleted test user as SUPER_ADMIN with password confirmation; verify audit log and `deletedAt = null`.
