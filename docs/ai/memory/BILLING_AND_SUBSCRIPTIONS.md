...
- Subscription model includes Stripe and mobile-store fields, plan/status/provider/platform, purchase-token hashes, App Store environment, grace/trial/current period, cancellation metadata, and versions.
...
- Production runtime startup currently runs migrations before server start; billing schema changes should not happen implicitly on app boot.
