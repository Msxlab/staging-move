...
4. Re-test Dossier PDF export from a production-like standalone build; the Docker build now runs the standalone prep that explicitly copies pdfkit standard font data.
...
6. Re-test the household invite modal focus/autofill behavior and the Home Dossier ambient animations/light-mode theme against the latest supplied theme reference.
7. Re-test Route Map light-mode labels; they should read as clean paper chips with endpoint tones, not fixed black tags.
