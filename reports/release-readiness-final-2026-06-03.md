...
- Code fails closed correctly while those are missing:
  ...
  - Play RTDN rejects production-like requests without audience/identity config.
...
Safe to merge the code hardening and Android build fixes after review.

Not safe to market-launch Android paid subscriptions until the missing Google Play Developer API/RTDN credentials are configured and verified. Not safe to advertise live partner auto-sync until connector runtime config/control-plane registration is enabled and partner agreements are complete.
